...
# RandomBackground(RANDOM_BG_COLOR_RANGE),
# RandomPermuteRGB(),
...
def my_collate_fn(samples):
    ...
    for k in keys:
        ...
        for sample in samples:
            if 'id' in k or 'label' in k:
                batch[k].append(sample[k])
            elif 'voxel' in k:
                batch[k].append(torch.from_numpy(sample[k].astype(np.float32)))
            else:
                if 's' in k:
                    s_img = sample[k]
                    # print('s_img', type(s_img))
                    s_img = s_tf(s_img)#.astype(np.float32))
                    
                    batch[k].append(s_img) #.transpose((1,2,0))))
                elif 'r' in k:
                    r_img = sample[k]
                    r_img = Image.fromarray(r_img.astype('uint8'), 'RGB')
                    # print('r_img', type(r_img))
                    r_img = r_tf(r_img)#.astype(np.float32).transpose((1,2,0)))
                    
                    batch[k].append(r_img)
...
def reshape_img(img):
    ...
    img = img.repeat(3, axis=0).astype(np.float32)
    img = Image.fromarray(img.astype('uint8').transpose(1,2,0))
    return img


def preprocess_s_img(img):
    ...
    return img.repeat(3, axis=0).astype(np.float32)


def preprocess_r_img(img):
    img = np.array(img)
    img = cv2.resize(img, dsize=(img_size, img_size), interpolation=cv2.INTER_CUBIC)
    # print(img.shape)

    img = img / 255
    var = [0.229, 0.224, 0.225]  # np.var(img, axis=(1, 2), keepdims=True)
    mean = [0.485, 0.456, 0.406]  # np.mean(img, axis=(1, 2), keepdims=True)
    
    if len(img.shape) == 2:
        # img = img.convert('RGB')
        img = np.expand_dims(img, 2)  # [1, H, W]
...
def get_pseudo_label(pseudo_label, subclass_id, r_img_id):
    reloc_id = pseudo_label['image_info'].index(subclass_id*1e5+r_img_id)
    ps_voxel = np.expand_dims(np.array(pseudo_label['pseudo_label'][reloc_id,::], 0))
